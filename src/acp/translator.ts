...
export class AcpGatewayAgent implements Agent {
  ...
  private pendingBySessionKey = new Map<string, string>(); // sessionKey -> sessionId
  ...
  handleGatewayDisconnect(reason: string): void {
    ...
    this.pendingBySessionKey.clear();
  }
  ...
  async prompt(params: PromptRequest): Promise<PromptResponse> {
    ...
    if (session.abortController) {
      ...
      // Tell the server to abort the previous run
      void this.gateway
        .request("chat.abort", { sessionKey: session.sessionKey })
        .catch((err) => this.log(`preemptive abort error: ${String(err)}`));
    }

    // Drain and reject any existing pending promise for this session
    const existingPending = this.pendingPrompts.get(params.sessionId);
    if (existingPending) {
      if (existingPending.timer) {
        clearTimeout(existingPending.timer);
      }
      this.pendingPrompts.delete(params.sessionId);
      this.pendingBySessionKey.delete(existingPending.sessionKey);
      existingPending.reject(new Error("Superseded by a new prompt request"));
    }
    ...
    return new Promise<PromptResponse>((resolve, reject) => {
      const timeoutMs = readNumber(params._meta, ["timeoutMs"]) ?? 5 * 60 * 1_000;
      const timer = setTimeout(() => {
        if (this.pendingPrompts.has(params.sessionId)) {
          this.pendingPrompts.delete(params.sessionId);
          this.pendingBySessionKey.delete(session.sessionKey);
      ...
      this.pendingBySessionKey.set(session.sessionKey, params.sessionId);
  ...
  async cancel(params: CancelNotification): Promise<void> {
    ...
    if (pending) {
      ...
      this.pendingPrompts.delete(params.sessionId);
      this.pendingBySessionKey.delete(pending.sessionKey);
  ...
  private findPendingBySessionKey(sessionKey: string): PendingPrompt | undefined {
    const sessionId = this.pendingBySessionKey.get(sessionKey);
