/**
 * @module agents/anthropic-payload-log
 *
 * Optional diagnostic logger that captures raw Anthropic API payloads and
 * usage statistics to a JSONL file on disk.
 *
 * When enabled via `OPENCLAW_ANTHROPIC_PAYLOAD_LOG=true`, every outbound
 * request payload is SHA-256-digested and written alongside run/session
 * metadata. After each LLM turn the last assistant message's usage stats
 * are also recorded. The log file path defaults to
 * `<stateDir>/logs/anthropic-payload.jsonl` but can be overridden with
 * `OPENCLAW_ANTHROPIC_PAYLOAD_LOG_FILE`.
 *
 * Only Anthropic-flavored models (api === "anthropic-messages") are logged;
...
/** Which phase of the request lifecycle this log event represents. */
