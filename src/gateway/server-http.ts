...
const HOOK_AUTH_FAILURE_TTL_MS = 15 * 60_000; // 15 minutes
const HOOK_AUTH_FAILURE_CLEANUP_INTERVAL_MS = 5 * 60_000; // 5 minutes
...
export function createHooksRequestHandler(
  ...
  // Periodically purge stale entries so dormant attacker IPs don't persist forever.
  const cleanupTimer = setInterval(() => {
    const now = Date.now();
    for (const [key, entry] of hookAuthFailures) {
      if (now - entry.windowStartedAtMs >= HOOK_AUTH_FAILURE_TTL_MS) {
        hookAuthFailures.delete(key);
      }
    }
  }, HOOK_AUTH_FAILURE_CLEANUP_INTERVAL_MS);
  ...
  const resolveHookClientKey = (req: IncomingMessage): string => {
    ...
    // Normalize IPv4-mapped IPv6 (::ffff:127.0.0.1 → 127.0.0.1) so both
    // representations count against the same throttle bucket.
