...
function ipMatchesCIDR(ip: string, cidr: string): boolean {
  ...
  // Validate prefix length — allow up to 128 for IPv6 CIDRs
  const maxPrefix = subnet.includes(":") ? 128 : 32;
  if (Number.isNaN(prefixLen) || prefixLen < 0 || prefixLen > maxPrefix) {
    return false;
  }
  ...
  const mask = prefixLen === 0 ? 0 : ((-1 >>> (32 - prefixLen)) << (32 - prefixLen)) >>> 0;
