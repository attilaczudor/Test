...
/**
 * Creates a new {@link AuthRateLimiter} with the given configuration.
 *
 * The returned limiter tracks per-IP failed authentication attempts within
 * a sliding time window. Once the maximum number of failures is reached,
 * the IP is locked out for a configurable duration. A periodic timer
 * prunes stale entries to prevent unbounded memory growth.
 *
 * @param config - Optional rate-limit configuration overrides.
 * @returns A fully initialised {@link AuthRateLimiter} instance.
 */
export function createAuthRateLimiter(config?: RateLimitConfig): AuthRateLimiter {
  ...
  const pruneTimer = setInterval(() => {
    try {
      ...
      console.error("[auth-rate-limit] Prune timer failed:", err);
    }
  }, PRUNE_INTERVAL_MS);
  ...
  function normalizeIp(ip: string | undefined): string {
    const resolved = (ip ?? "").trim();
    if (!resolved) {
      console.warn(
        "[auth-rate-limit] Request received with no IP address; rate limiting may not work correctly."
      );
      return "unknown";
    }
    return resolved;
