  ...
  async function runCheck() {
    ...
    try {
      ...
      for (const [channelId, accounts] of Object.entries(snapshot.channelAccounts)) {
        ...
        const typedChannelId = channelId as ChannelId;
        for (const [accountId, status] of Object.entries(accounts)) {
          ...
          if (channelManager.isManuallyStopped(typedChannelId, accountId)) {
            continue;
          }
          ...
          try {
            if (status.running) {
              await channelManager.stopChannel(typedChannelId, accountId);
            }
            // Re-check state after stopChannel() to avoid races where another
            // fiber fixed the channel while we were stopping it.
            const freshSnapshot = channelManager.getRuntimeSnapshot();
            const freshAccounts = freshSnapshot.channelAccounts[channelId];
            const freshStatus = freshAccounts?.[accountId];
            if (freshStatus && isChannelHealthy(freshStatus)) {
              // Channel is now healthy (another fiber restarted it) — skip.
              continue;
            }
            channelManager.resetRestartAttempts(typedChannelId, accountId);
            await channelManager.startChannel(typedChannelId, accountId);
