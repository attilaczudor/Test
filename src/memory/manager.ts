      ...
      const tryAcquire = () => {
        if (!this.locked) {
          ...
          const release = (): void => {
            ...
            if (next) {
              next(); // locked stays true; next waiter inherits the lock
            } else {
              this.locked = false;
            }
          };
...
/**
 * Bounded LRU cache for MemoryIndexManager instances.
 * Evicts the least-recently-used entry when the max size is reached,
 * preventing unbounded memory growth when many agent/workspace combos are used.
 */
class LRUCache<K, V> {
  ...
  get(key: K): V | undefined {
    ...
    // Refresh access order: delete then re-insert moves entry to end (MRU position).
    this.map.delete(key);
  ...
  set(key: K, value: V): void {
    if (this.map.has(key)) {
      ...
      log.warn(
        `[memory] INDEX_CACHE reached max size (${this.maxSize}); evicting LRU entry for key: ${String(lruKey)}`
      );
    }
    this.map.set(key, value);
  }
...
const MAX_INDEX_CACHE_SIZE = 100;
const INDEX_CACHE = new LRUCache<string, MemoryIndexManager>(MAX_INDEX_CACHE_SIZE);
...
export class MemoryIndexManager extends MemoryManagerEmbeddingOps implements MemorySearchManager {
  ...
  async sync(params?: {
    ...
    let existing: Promise<void> | undefined;
    try {
      if (this.closed) {
        return;
      }
      if (this.syncing) {
        existing = this.syncing;
        return;
      }
    ...
    if (!allowedWorkspace && this.settings.extraPaths.length > 0) {
      ...
      for (const additionalPath of additionalPaths) {
        try {
          ...
          log.warn(`memory readFile: could not stat extra path: ${String(err)}`);
        }
      }
    }
  ...
  async close(): Promise<void> {
    ...
    if (pendingSync) {
      try {
        ...
        log.debug(
          `pending sync error during close: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
    this.db.close();
