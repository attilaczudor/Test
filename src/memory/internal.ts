...
export function ensureDir(dir: string): string {
  try {
    ...
    // Directory may already exist or be created concurrently; safe to ignore.
  }
  return dir;
}
...
export async function listMemoryFiles(
  ...
  try {
    ...
    // Memory directory does not exist — skip
  }
  ...
  for (const entry of result) {
    ...
    try {
      ...
      // realpath failed — use original path for dedup key
    }
    if (seen.has(key)) {
      continue;
    }
