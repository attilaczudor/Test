/**
 * @module browser-cli-debug
 *
 * Registers browser debugging CLI subcommands: `highlight`, `errors`, `requests`,
 * and `trace` (start/stop).
 *
 * These commands provide visibility into the browser's runtime behavior:
 * - `highlight` - Visually highlight a DOM element by its snapshot ref ID
 * - `errors` - Retrieve recent page errors (JavaScript exceptions)
 * - `requests` - List recent network requests (with optional URL filter)
 * - `trace start` - Begin recording a Playwright trace
 * - `trace stop` - Stop trace recording and write the .zip file
 */
...
/**
 * Executes a browser debug action with standardized error handling.
 *
 * @param action - The async debug action to execute
 */
...
/**
 * Normalizes debug query parameters by trimming strings, converting booleans,
 * and stripping empty/undefined values.
 *
 * @param params - Raw query parameters from CLI options
 * @returns A cleaned query object suitable for the browser API call
 */
...
/**
 * Registers browser debug subcommands (`highlight`, `errors`, `requests`, `trace`)
 * on the given parent browser command.
 *
 * @param browser - The parent `browser` Commander command
 * @param parentOpts - Accessor function to retrieve parent command options
 */
